...
Fly to [F Ironforge]
[G50.82,5.61Ironforge]Turn in [QT968 The Powers Below] if you have it[O]
[G72.08,51.87Ironforge]Turn in [QT1072 An Old Colleague][O]
[G67.84,42.5Ironforge]Buy a [V]*Bronze Tube* if you haven't already[O]--OnStepActivation,BAG_UPDATE>>BronzeTube
Take the tram to Stormwind-->>ZoneSkip,Stormwind City,1
...
function Guidelime.Zarant:BronzeTube(args)
	local reverseLogic,skip = unpack(args)
	if reverseLogic ~= "1" then
		reverseLogic = false
	end
	if skip then
		skip = tonumber(args[2])
	end
	
	if not reverseLogic == (IsQuestFlaggedCompleted(174) or GetItemCount(4371) > 0) then
		self:SkipStep()
	elseif skip and not IsQuestFlaggedCompleted(57) then
		self:SkipStep()
		self:LoadNextGuide(skip)
	end
end